...
NSString *const TGFileCacheDomain = @"com.neo.FileCache";
...
- (instancetype)init
{
    return [self initWithName:nil useMemoryCache:YES];
}
...
- (instancetype)initWithName:(NSString *)name useMemoryCache:(BOOL)useMemoryCache
{
    ...
    if (self != nil)
    {
        ...
        _queue = dispatch_queue_create(TGFileCacheDomain.UTF8String, NULL);
        ...
        dispatch_async(_queue, ^{
            ...
            {
                [[NSFileManager defaultManager] createDirectoryAtURL:_url withIntermediateDirectories:YES attributes:nil error:nil];
            }
        });
    }
    return self;
...
{
    [self fetchDataForKey:key memoryOnly:NO synchronous:synchronous unserializeBlock:unserializeBlock completion:completion];
}
...
- (void)cacheData:(NSObject<NSCoding> *)data key:(NSString *)key synchronous:(BOOL)synchronous serializeBlock:(NSData *(^)(NSObject<NSCoding> *))serializeBlock completion:(void (^)(NSURL *))completion
{
    void (^block)(void) = ^{
        ...
        [serializedData writeToURL:url atomically:YES];
        if (completion != nil)
            completion(url);
    };
